#!/bin/zsh
# vi2meta.sh - the one line you paste into VoiceInk's Custom Command field.
#
...
#
#     /path/to/prototype/vi2meta.sh --mode email
#     /path/to/prototype/vi2meta.sh --mode notes
#
#   Without --mode, meta.json simply omits modeName and triggerModes will not match.

set -u

# Resolve this script's own directory (following symlinks) so the vi2meta package is
# importable no matter what working directory VoiceInk happens to invoke us from.
HERE="${0:A:h}"
export PYTHONPATH="$HERE${PYTHONPATH:+:$PYTHONPATH}"

exec /usr/bin/env python3 -m vi2meta "$@"
